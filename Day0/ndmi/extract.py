...
def process_ndbi_image(sentinel_image, geometry):
    """
    📌 Why this step?
    Sentinel-2 images contain 13 bands, including infrared, water vapor, etc.
    But for NDBI calculation, we need specific bands:

    Band Name	Meaning	Resolution
    B11	Short-wave Infrared (SWIR)	20m
    ...
    ✅ Bands Needed for NDBI:
    1.B11 → Short-wave Infrared (SWIR)
    2.B8 → Near-Infrared (NIR)

    NDBI stands for:

    1.Normalized Difference Built-up Index
    2.It is a formula to measure how built-up (urban) a place is.

    1.Why Do We Use It?
    2.Because:
    3.Built-up areas (concrete, asphalt, buildings) reflect more SWIR light
    4.Vegetation and water reflect more NIR light
    5.So we compare how much SWIR vs NIR is coming from a pixel.

    NDBI = (SWIR - NIR) / (SWIR + NIR)
     
    """
    logger.info("Processing NDBI bands...")
    
    # Select NDBI bands and clip to Sector 14
    ndbi_image = (
        sentinel_image
        .select(['B11', 'B8'])  # SWIR and NIR
    ...
    '''
    #  Prepare NDBI image for export with brightness correction
    ...
    ndbi_scaled = (
        ...
        .uint8()            # Convert to unsigned 8-bit integers, required for exportable image formats
    )

    '''
    ndbi_scaled = ndbi_image.multiply(0.0001).pow(0.5).multiply(255).uint8()
    
    logger.info("NDBI image processed successfully")
    return ndbi_scaled
...
def main():
    ...
    try:
        ...
        # Process NDBI image
        ndbi_scaled = process_ndbi_image(sentinel_image, sector14_geom)
        
        # Export image to Google Drive
        task = export_image_to_drive(ndbi_scaled, date_used, sector14_geom)
