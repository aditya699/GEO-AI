...
def process_mndwi_image(sentinel_image, geometry):
    """
    🔍 What is MNDWI?
    ...
    But for MNDWI calculation, we need specific bands:
    ...
    ✅ Bands Needed for MNDWI:
    1.B3 → Green
    2.B11 → Short-wave Infrared (SWIR)
     
    """
    logger.info("Processing MNDWI bands...")
    
    # Select MNDWI bands and clip to Sector 14
    mndwi_image = (
        sentinel_image
        .select(['B3', 'B11'])  # Green and SWIR
    ...
    '''
    #  Prepare MNDWI image for export with brightness correction
    ...
    '''
    mndwi_scaled = mndwi_image.multiply(0.0001).pow(0.5).multiply(255).uint8()
    
    logger.info("MNDWI image processed successfully")
    return mndwi_scaled
...
def main():
    ...
    try:
        ...
        # Process MNDWI image
        mndwi_scaled = process_mndwi_image(sentinel_image, sector14_geom)
        
        # Export image to Google Drive
        task = export_image_to_drive(mndwi_scaled, date_used, sector14_geom)
